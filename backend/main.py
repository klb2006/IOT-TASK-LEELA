...
async def predict_water_activity(request: dict):
    """
    Predict water activity/usage type using GRU model (model32.h5)
    
    Detects activities:
    - no_activity: Tank is idle, no water usage
    - filling: Water is being added to tank
    - flush: Sudden change in water level (toilet/flush)
    - washing_machine: Extended water usage pattern
    - geyser: Hot water usage pattern
    
    Request body (from IoT device):
    {
        "distance": 50.0,              # Current distance from ultrasonic sensor (cm)
        "temperature": 28.5,            # Current temperature (°C)
        "node_id": "node-1"             # Optional node identifier
    }
    
    Response:
    {
        "status": "success",
        "activity": "filling | flush | washing_machine | geyser | no_activity",
        "confidence": 0.85,             # Confidence score 0-1
        "distance": 50.0,
        "temperature": 28.5,
        "probabilities": {              # Optional breakdown of all classes
            "no_activity": 0.05,
            "filling": 0.85,
            "flush": 0.05,
    ...
    """
    try:
        from ml_training.activity_classifier import predict_activity as classify_activity
        from ml_training.activity_classifier import get_activity_info
        
        # Extract parameters
        distance = request.get('distance')
        temperature = request.get('temperature', 25.0)
        node_id = request.get('node_id', 'node-1')
        return_probs = request.get('return_probabilities', True)
        
        # Validate inputs
        if distance is None:
            return {
                "status": "error",
                "message": "Missing required field: distance (cm from ultrasonic sensor)"
            }
        ...
        if result['status'] == 'success':
            insert_prediction(
                ...
                distance=float(distance),
                temperature=float(temperature),
                water_percent=0,  # Not applicable for activity detection
        ...
        return {
            "status": "error",
            "message": f"Activity classifier module not available: {str(e)}",
            "hint": "Ensure activity_classifier.py exists in ml_training/"
        }
    except Exception as e:
        import traceback
        traceback.print_exc()
        return {
            "status": "error",
            "message": f"Activity prediction failed: {str(e)}"
        }

@app.get("/api/v1/activity-model-info")
async def get_activity_model_info():
